...
- See [jsonoutput_test.go](jsonoutput_test.go) for example code. I left the
  `t.Parallel()` in, as this is how my production tests are written. But this is
  not required to trigger the JSON output.
...
- See [marshaloutput_test.go](marshaloutput_test.go) for example code, see the
  `// FIXME` comments.
