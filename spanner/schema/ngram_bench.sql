...
CREATE TABLE AlbumsNgramMin3 (
    AlbumId INT64 NOT NULL,
    Title STRING(1024),
    Title_Tokens TOKENLIST AS (
        TOKENIZE_SUBSTRING(Title, ngram_size_min=>3, ngram_size_max=>3,
            relative_search_types=>["word_prefix", "word_suffix"])
    ) HIDDEN
) PRIMARY KEY (AlbumId);

CREATE SEARCH INDEX AlbumsNgramMin3Index
    ON AlbumsNgramMin3(Title_Tokens)
    STORING (Title);
